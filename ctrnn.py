...
class CTRNN():
    ...
    def setConstantParameters(self):
        self.Weights = np.random.uniform(10,10,size=(self.Size,self.Size))
        self.Biases = np.random.uniform(10,10,size=(self.Size))
        self.TimeConstants = np.random.uniform(5.0,5.0,size=(self.Size))
        self.invTimeConstants = 1.0/self.TimeConstants
    ...
    def step(self, dt):
        ...
        #         self.Weights[i][j] += dt * (self.learningRate * self.Outputs[i] * self.Outputs[j])
